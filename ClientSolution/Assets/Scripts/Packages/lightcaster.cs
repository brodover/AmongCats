...
public class lightcaster : MonoBehaviour
{
	...
	void Start ()
    {
        ignoreMe = LayerMask.GetMask("OtherSide");
        wallMask = LayerMask.GetMask("Wall");

        lightRays = GameObject.FindGameObjectWithTag("LightMask");
    ...
    {
        Collider[] colliders = Physics.OverlapSphere(transform.position, getRadius, wallMask);
        sceneObjects = colliders.Select(collider => collider.gameObject).ToArray();
    }
}